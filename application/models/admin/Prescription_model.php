...
*/
 	public function patient_info($appointment_id)
    {
        ...
        $this->db->join('patient_tbl', 'patient_tbl.patient_id = appointment_tbl.patient_id','left'); 
        $this->db->join('doctor_tbl', 'doctor_tbl.doctor_id = appointment_tbl.doctor_id','left'); 
        $this->db->join('venue_tbl', ' venue_tbl.venue_id = appointment_tbl.venue_id','left'); 
...
*/
    public function prescription_list()
    {
        ...
        $this->db->join('patient_tbl', 'patient_tbl.patient_id = prescription_data.patient_id','left'); 
        $query = $this->db->get();
        $result = $query->result();
        return $result;
    }
...
*/
    public function chackExis($appointment_id)
    {
            ...
            patient_tbl.address as ads,
             ...
             $this->db->join('medicine_prescription', 'medicine_prescription.prescription_id = prescription.prescription_id','left'); 
             $this->db->join('medecine_info', 'medecine_info.medicine_id = medicine_prescription.medicine_id','left'); 
   
             $this->db->join('patient_tbl', 'prescription.patient_id = patient_tbl.patient_id','left'); 
             $this->db->join('doctor_tbl', 'prescription.doctor_id = doctor_tbl.doctor_id','left'); 
...
*/
    ...
    public function prescription_by_id($prescription_id){
         return $result = $this->db->select("prescription.*,
            ...
            ->from("prescription")
            ->join('medicine_prescription', 'medicine_prescription.prescription_id = prescription.prescription_id','left') 
            ->join('medecine_info', 'medecine_info.medicine_id = medicine_prescription.medicine_id','left') 
            ->join('patient_tbl', 'prescription.patient_id = patient_tbl.patient_id','left') 
            ->join('doctor_tbl', 'prescription.doctor_id = doctor_tbl.doctor_id','left') 
    ...
    public function gereric_by_id($prescription_id){
         return $result = $this->db->select("prescription.*,
             ...
             log_info.email,log_info.log_id")
            ->from("prescription")
            ->join('generic_tbl', 'generic_tbl.prescription_id = prescription.prescription_id','left') 
            ->join('medicine_group_tbl', 'medicine_group_tbl.med_group_id = generic_tbl.group_id','left') 
            ->join('patient_tbl', 'prescription.patient_id = patient_tbl.patient_id','left') 
            ->join('doctor_tbl', 'prescription.doctor_id = doctor_tbl.doctor_id','left') 
            ->join('log_info', 'log_info.log_id = doctor_tbl.log_id','left')
            ->where('prescription.prescription_id',$prescription_id)
